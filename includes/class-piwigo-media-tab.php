...
class Piwigo_Media_Tab
{
  public static function admin_init(): void
  {
    ...
    add_action('media_buttons',         array(self::class, 'classic_editor_button'));
  }

  public static function classic_editor_button(): void
  {
    if (!get_option('piwigo_media_server_url', '')) {
      return;
    }
    $icon = esc_url(PIWIGO_MEDIA_URL . 'assets/images/piwigo-black.svg');
    printf(
      '<button type="button" id="insert-from-piwigo" class="button">'
        . '<img src="%s" width="18" height="18" alt="" style="vertical-align:middle"> %s'
      . '</button>',
      $icon,
      esc_html__('Add from Piwigo', 'piwigo-media')
    );
  }
  ...
  public static function enqueue(string $hook): void
  {
    ...
    wp_localize_script('piwigo-media-frame', 'piwigoMediaConfig', $config);
  }
}
